...
function ratingTemplate(rating = 0) {
  ...
  let html = `<p class="recipe__ratings" role="img" aria-label="${ariaLabel}">`;

  for (let i = 1; i <= 5; i++) {
    const starClass = i <= Math.floor(rating) ? 'icon-star' : 'icon-star-empty';
    const starChar = i <= Math.floor(rating) ? '⭐' : '☆';
    html += `<span aria-hidden="true" class="${starClass}">${starChar}</span>`;
  }
...
function recipeTemplate(recipe) {
  return `<figure class="recipe card">
    ...
    <figcaption class="info">
      <ul class="recipe__tags tag">${tagsTemplate(recipe.tags)}</ul>
...
function renderRecipes(list) {
  ...
  if (!list.length) {
    main.innerHTML = `<p>No recipes found. Try another search.</p>`;
    return;
  }
  main.innerHTML = list.map(recipeTemplate).join('');
}
...
function filterRecipes(query) {
  ...
  return recipes.filter(recipe => {
    return keywords.some(keyword => {
      ...
      const ingredientMatch = recipe.recipeIngredient.some(i => i.toLowerCase().includes(keyword));
      const tagMatch = recipe.tags.some(t => t.toLowerCase().includes(keyword));
...
function searchHandler(e) {
  ...
  renderRecipes(filterRecipes(input.value.trim()));
}

function init() {
  renderRecipes([getRandomEntry(recipes)]);
